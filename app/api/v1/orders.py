...
@router.post('/', response_model=OrderFromDB, response_model_by_alias=False, status_code=status.HTTP_201_CREATED, dependencies=[Depends(RoleChecker(['admin']))])
...
@router.get('/', response_model=list[OrderFromDB], response_model_by_alias=False, status_code=status.HTTP_200_OK, dependencies=[Depends(RoleChecker(['admin']))])
...
@router.get('/{order_id}', response_model=OrderFromDB, response_model_by_alias=False, status_code=status.HTTP_200_OK, dependencies=[Depends(RoleChecker(['admin']))])
...
@router.patch('/{order_id}/status', response_model=OrderFromDB, response_model_by_alias=False, status_code=status.HTTP_200_OK, dependencies=[Depends(RoleChecker(['admin']))])
